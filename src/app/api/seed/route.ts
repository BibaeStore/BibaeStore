...
export async function GET() {
  ...
  const { data, error } = await supabase.from('blog_posts').upsert([{
    title: 'What to Wear on a Winter Wedding in Pakistan: 5 Simple Ideas',
    ...
    `,
    cover_image: 'https://images.unsplash.com/photo-1595777457583-95e059f581ce?q=80&w=1200&auto=format&fit=crop',
    author: 'Habiba Minhas Team',
    meta_title: 'What to Wear on a Winter Wedding in Pakistan: 5 Simple Ideas',
    meta_description: 'Struggling to find the perfect winter wedding outfit? Discover 5 elegant and simple ideas to stay warm without sacrificing your style at Pakistani winter weddings.',
    keywords: ['winter wedding', 'pakistani fashion', 'velvet dresses', 'wedding guest outfits', 'khaddar'],
    category: 'Formal Wear & Wedding Fashion',
    status: 'published'
  }], { onConflict: 'slug' }).select();

  if (error) {
    if (error.code === '42P01') {
      return NextResponse.json({ success: false, message: 'The blog_posts table does not exist in Supabase! You must go to Supabase Dashboard and run create_blog_table.sql inside the SQL Editor.' });
    }
    return NextResponse.json({ success: false, error });
  }

  return NextResponse.json({ success: true, message: 'Blog seeded successfully!', data });
}
